...
import { Plus, Pencil, Snowflake, X, ChevronDown, ChevronUp, Trash2, CloudSun } from 'lucide-react';
import './meiaestacao.css';

export default function MeiaEstacao() {
  const [isModalAberto, setIsModalAberto] = useState(false);
  const [produtoEditando, setProdutoEditando] = useState(null);
  const [itensExpandidos, setItensExpandidos] = useState({});
  
  const [produtosmeiaestacao, setProdutosmeiaestacao] = useState(() => {
    const salvo = localStorage.getItem('produtos_meiaestacao_json');
  ...
  useEffect(() => {
    localStorage.setItem('produtos_meiaestacao_json', JSON.stringify(produtosmeiaestacao));
  }, [produtosmeiaestacao]);
  ...
  const handleAbrirCadastro = () => {
    let proximoNumero = 1;
    if (produtosmeiaestacao.length > 0) {
      const numerosAtuais = produtosmeiaestacao.map(p => parseInt(p.itemCodigo.replace('I', ''), 10) || 0);
  ...
  const handleSalvarItem = (e) => {
    e.preventDefault();
    if (produtoEditando) {
      setProdutosmeiaestacao(produtosmeiaestacao.map(p => p.id === produtoEditando.id ? novoProduto : p));
    } else {
      setProdutosmeiaestacao([...produtosmeiaestacao, { ...novoProduto, id: Date.now() }]);
    }
    ...
    <div className="estoque-container">
      <header className="estoque-header">
        <div className="header-titulo">
          <CloudSun size={24} color="#1E3A8A" />
          <h1>Modelos Meia Estação ({produtosmeiaestacao.length})</h1>
        </div>        
      ...
      <div className="overflow-lista">
        <div className="lista-estoque">
          ...
          <div className="lista-header grid-tabela-meiaestacao">
            <div></div>
            <div>Item</div>
            <div>Modelo</div>
          ...
          {produtosmeiaestacao.map((prod) => {
            ...
            return (
              <div key={prod.id} className="container-item-meiaestacao">
                
                <div className="lista-item grid-tabela-meiaestacao" onClick={() => toggleExpandir(prod.id)} >
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              <h2>{produtoEditando ? `Editar ${produtoEditando.itemCodigo}` : "Novo Produto meiaestacao"}</h2>
